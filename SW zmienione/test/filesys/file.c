...
/* ****************************************************************************  
 * esint16 file_setpos(File *file,unsigned long pos)
...
unsigned long file_fwrite(File* file,unsigned long offset,unsigned long size,unsigned char* buf)
{
	unsigned long need_cluster;
	unsigned long cclus,csec,cbyte;
	unsigned long size_left=size,bytes_written=0;
	unsigned long rclus,rsec;
	unsigned long coffset=offset;
	unsigned short btr;
	unsigned char *tbuf;
...
/* ***************************************************************************\
 * signed char file_fopen(FileSystem *fs,File* file,char* filename)      
...
void file_initFile(File *file, FileSystem *fs, FileLocation *loc)
{
	...
	file->Cache.Linear=0;
	file->Cache.FirstCluster=(((unsigned long)file->DirEntry.FirstClusterHigh)<<16)+ 
...
{
	unsigned long clusters_required,clustersize;
	unsigned long hc;
