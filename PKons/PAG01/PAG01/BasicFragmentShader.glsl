...
void main()
{
	...
	float lightPower = clamp(dot(-normalize(lightDirection).xyz, normalize(normal)), 0.0f, 1.0f);
	vec3 diff = POWER_CORRECTION * lightPower * lightDiffuse.xyz * highlight.xyz;

	vec3 r = normalize(reflect(-normalize(lightDirection.xyz), normalize(normal)));
