...
void main()
{
	...
	{
		vec3 bMin = vec3(texelFetchBuffer(InBaaCols, i * 2));
		vec3 bMax = vec3(texelFetchBuffer(InBaaCols, i * 2 + 1));
	...
	{
		vec4 sphere = vec4(texelFetchBuffer(InSCols, i));
	...
	for (int i = 0; i < 4; ++i)
	{
		vec3 wnPos = vec3(WorldMatrix * vec4(texelFetchBuffer(InPos, int(Neighbours[i]))));
	...
	OutPos = finalPos;
	OutPosLast = PosLast;
	//OutPosLast = vec4(texelFetchBuffer(InBaaCols, 1));
	gl_Position = Pos;
}