/*---------------------------------------------------------
*	Name: SPI_MMC.C
*	Purpose: SPI mode SD/MMC card interface module
*-------------------------------------------------------*/
...
void SPI_Send(BYTE* buf, DWORD Length)
{
	...
	while (Length != 0)
	{
		/*as long as TNF is set, TxFIFO is not full, I can write*/
		while (!(SSPSR & 0x02));
		SSPDR = *buf;
...
int mmc_init()
{
	...
	/*Initialize the MMC card into SPI mode by sending 80 clks on*/
	...
	/*send CMD0 (RESET or GO_IDLE_STATE) command, all the arguments are 0x00 for the reset command,
	...
	/*must keep sending command until zero resposne is back*/
	i = MAX_TIMEOUT;
	do
	{
	...
	/*send some dummy clocks after SEND_OF_COND*/
	IOSET0 = SPI_SEL;
	SPI_ReceiveByte();
	IOCLR0 = SPI_SEL;
...
int mmc_write_block(WORD block_number)
{
	...
	/*send mmc CMD24(WRITE_SINGLE_BLOCK) to write data to MMC Card*/
	MMCCmd[0] = 0x58;
...
int mmc_read_block(WORD block_number)
{
	...
	SPI_Receive(MMRDData, MMC_DATA_SIZE);
