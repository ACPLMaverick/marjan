...
signed short fs_verifySanity(FileSystem *fs)
{
	signed short sane=1; /* Sane until proven otherwise */
...
void fs_countDataSectors(FileSystem *fs)
{
  unsigned long rootDirSectors,dataSectorCount;
...
/* ****************************************************************************  
 * long fs_clusterToSector(FileSystem *fs,unsigned long cluster)
...
signed char fs_findFile(FileSystem *fs,char* filename,FileLocation *loc,unsigned long *lastDir)
{
	unsigned long fccd,tmpclus;
	char ffname[11],*next,it=0,filefound=0;
