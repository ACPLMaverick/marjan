	...
	bool fullscreen, float screenDepth, float screenNear)
{
	...
	//Now go through all the display modes and finf the one that matches the screen width and height
	...
	/* Direct3D device and Direct3D device context are very important, 
	...
	//Create the render target view with the back buffer pointer (attaches the back buffer to swap chain)
	...
	//Initialize the description od the depth buffer (so that our polygons can be rendered properly in 3D space)
	...
	//Setup the projection matrix (used to translate the 3D scene into the 2D viewport space that we previously created)
	...
	//View matrix is created in camera class
	//Is used to calculate the position of where we are looking at the scene from.
...
void D3DClass::Shutdown() /* will release and clean up all the pointers used in the Initialize function*/
{
...
/*Will be called whenever we are going to draw a new 3D scene at the beginning of each frame.
...
void D3DClass::EndScene() /*It tells the swap chain to display our 3D scene once all the drawing has completed at the end of each frame.*/
{
